...
export default function HomeClientWrapper({ heroHeading, heroSupport, staticContent }: HomeClientWrapperProps) {
    ...
    useEffect(() => {
        ...
        if (!mediaQuery.matches || reducedMotion || saveData) {
            return;
        }
        ...
        if ('requestIdleCallback' in window) {
            const idleId = window.requestIdleCallback(loadVideo, { timeout: 2200 });
        ...
        <div className="min-h-screen bg-background">
            ...
            >
                <div className="pointer-events-none absolute inset-0">
                    <Image
                        src={mediaAsset("/images/gallery/custom/seedance-hero-w8ki0.png")}
                        alt=""
                        fill
                        priority
                        sizes="100vw"
                        className={`object-cover opacity-34 transition-opacity duration-700 ${shouldLoadHeroVideo ? 'md:opacity-0' : ''}`}
                    />
                    {shouldLoadHeroVideo ? (
                        <video
                            autoPlay
                            ...
                            playsInline
                            preload="none"
                            className="hidden h-full w-full object-cover opacity-34 md:block"
